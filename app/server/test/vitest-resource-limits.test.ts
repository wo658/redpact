...
test.runIf(process.env.REDPACT_DOCKER_TESTS === "1")(
  "Vitest 실행은 선택한 한도로 무한 루프를 종료하고 다음 실행은 새 설정을 읽는다",
  async () => {
    ...
    try {
      const result = await runner.execute(
        {
          files: [
            {
              path: "loop.test.ts",
              source: `import { test } from 'vitest'; import { writeFileSync } from 'node:fs'; test('동기 루프', () => { writeFileSync('/review/output/started', 'loop-started'); while(true) {} })`,
            },
          ],
        } as never,
        "loop",
      ...
      expect(await readFile(join(root, "runs/loop/started"), "utf8")).toContain("loop-started")
