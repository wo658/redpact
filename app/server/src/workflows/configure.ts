...
export const configureInput = z.strictObject({
  action: z
    ...
    .describe(
      "describe returns the schema, examples and file-authoring workflow; inspect reads normalized settings, diagnostics and discovered tests; validate checks authored files and the fixed execution plan without writing or executing.",
    ),
})
  ...
  if (action === "describe") {
    return {
      ...
      nextSteps: [
        ...
        "Write the project-wide .redpact/settings.json in rulesRoot, then call configure validate. All worktrees use this fixed configuration; execution files resolve in the selected checkout.",
      ],
    }
  }
...
export async function configure(
  ...
  const result = await configureSettings(settings, input.action, Boolean(services.environments))
