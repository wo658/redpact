...
- 브라우저 데스크톱은 사이드바를 연 상태로 유지하며, 헤더에는 사이드바 전환, 페이지
  제목이나 이력 제어 대신 열린 작업공간 탭만 둡니다. 좁은 모바일 화면과 native macOS
  데스크톱에서는 프로젝트 선택 옆, 프로젝트 표시 옵션 바로 왼쪽에 사이드바 전환을 둡니다.
  닫힌 상태에서는 헤더 아래에 다시 열기 버튼이 나타납니다. Worktrees 표시 옵션은
  Worktrees 전체 영역의 호버, 키보드 포커스, 메뉴가 열린 동안 또는 호버가 없는 입력에서
  나타납니다. 작업공간 탭은 프로젝트와 현재 위치를 슬래시로 구분하고 중간 브랜치
  구간은 말줄임표로 축약합니다. 전체 위치는 접근성 이름과 툴팁으로 확인할 수 있습니다.
  너비 제한과 말줄임 규칙은 [헤더 레이아웃](frontend.md)을 참고하세요.
  각 작업공간 탭은 프로젝트·워크트리와 별개의 ID를 가지며 현재 페이지, 프로젝트와
  필요한 워크트리 또는 브랜치를 기록합니다. 사이드바의 페이지·프로젝트·워크트리·브랜치
  선택은 활성 탭 안에서 이동합니다. 다른 탭을 직접 선택해야 그 탭이 활성화되며,
  여러 탭이 같은 대상을 독립적으로 표시할 수 있습니다. 제목은 현재 페이지와 대상을 따릅니다.
- 헤더 더하기 버튼은 현재 페이지 위치를 새 탭으로 열고 상세 선택 상태는 초기화합니다.
  다른 프로젝트를 선택하면 현재 탭 안에서 해당 프로젝트의 기본 리뷰 페이지를 엽니다.
  각 탭은 탭 전환 중 사이드바와 현재 화면 상태를 유지합니다. 폴더 펼침, 파일 선택,
  Test 하위 탭, 작성 중인 입력과 스크롤 위치를 포함합니다. 다른 페이지로 이동하면
  이전 페이지의 상세 상태가 해제될 수 있으며 탭별 탐색 이력은 제공하지 않습니다.
  숨긴 탭은 구독을 중지하고 다시 활성화할 때 갱신합니다. 탭을 닫으면 해당 상태를
  해제합니다. 활성 탭을 닫으면 앞 탭을 선택하고 첫 탭이면 다음 탭을 선택합니다.
  비활성 탭을 닫으면 현재 활성 탭을 유지하며, 마지막 탭은 닫지 않습니다.
  닫기는 실행을 중지하거나 프로젝트 연결을 해제하지 않습니다. 탭과 페이지 위치는
  세션에만 유지되며 새로고침 또는 앱 재시작 시 초기화됩니다. 고정, 순서 변경과
  전체 UI 상태의 영구 저장은 지원하지 않습니다.
