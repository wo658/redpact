...
native macOS는 사이드바 전환을 제공하고, 좁은 모바일 화면에서도 전환 버튼을 유지합니다.
브라우저와 모바일에서는 열린 사이드바의 프로젝트 선택 옆, 프로젝트 표시 옵션 바로 왼쪽에
전환 버튼을 둡니다. 닫힌 상태에서는 작업공간 헤더 아래에 다시 열기 버튼이 나타납니다.

native macOS에서는 사이드바 토글과 뒤로·앞으로 버튼을 신호등 예약 영역 바로 다음,
사이드바의 48px 타이틀바 헤더에 고정합니다. 사이드바를 접어도 버튼 위치는 바뀌지 않습니다.
