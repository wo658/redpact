...
소유합니다. 작업공간 탭은 `프로젝트 / 사이드바 페이지`를 표시하며 워크트리나 브랜치 리뷰는
`프로젝트 / Worktrees`로 표시합니다. 워크트리 이름, 브랜치 경로와 내부 섹션은 의도적으로
생략합니다. 같은 레이블을 툴팁, 접근성 이름과 닫기 버튼에도 사용합니다.
사이드바 탐색과 브라우저 방문 기록에 따라 위치가 갱신되고, 세부 선택은 본문 도구 모음에 남습니다.
탭은 최대 256px이며 사용 가능한 탭 영역보다 넓어지지 않습니다. 프로젝트는 레이블 너비의
최대 40%를 사용하고 나머지 공간은 사이드바 페이지에 배정하며 각각 말줄임 처리합니다.
아이콘과 제목은 왼쪽 정렬하고 얇은 윤곽선을 사용합니다.
선택한 탭은 라이트·다크 테마 모두 강조색 배경과 더 선명한 기본색 테두리로 표시하며,
키보드 포커스는 별도의 링으로 구분합니다.
옆의 더하기 버튼은 현재 페이지 위치를 상세 상태가 초기화된 새 탭으로 열며 가로 스크롤 중에도 접근할 수 있습니다.
탭 선택이나 닫기 후에는 활성 탭이 보이도록 이동합니다. 여러 탭이 공간을 초과하면
탭 영역 안에서 가로로 스크롤합니다. 브라우저 데스크톱과
native macOS/Windows는 사이드바 전환을 제공하고, 좁은 모바일 화면에서도 전환 버튼을 유지합니다.
브라우저와 모바일에서는 열린 사이드바의 프로젝트 선택 옆, 프로젝트 표시 옵션 바로 왼쪽에
전환 버튼을 둡니다. 닫힌 상태에서는 작업공간 헤더 아래에 다시 열기 버튼이 나타납니다.

native macOS와 Windows는 사이드바 토글과 뒤로·앞으로 버튼을 하나의 48px 타이틀바
헤더에 배치합니다. macOS는 왼쪽에 시스템 신호등 영역을 확보하고, Windows는 왼쪽에
탐색 버튼, 오른쪽에 최소화·최대화/복원·닫기 버튼을 배치합니다. Windows 기본 제목
표시줄을 대체하고 네이티브 메뉴 표시줄을 숨깁니다. 서버 연결 중이나 연결 실패 화면에서도
창 버튼을 사용할 수 있습니다. 닫기는 기존처럼 창을 숨기며 네이티브 종료 단축키는 별개입니다.
헤더의 빈 공간을 드래그하면 창이 이동하고 더블클릭하면 최대화를 전환합니다.
Windows 최대화 버튼은 크기 변경과 포커스 복귀 시 상태를 반영합니다. Windows 11의
최대화 버튼 호버 Snap Layouts 메뉴는 구현하지 않습니다. 네이티브 드래그, 크기 조절,
단축키와 창 스냅은 Chromium 레이아웃 테스트와 별도로 Windows에서 검증해야 합니다. 사이드바를 접어도 버튼 위치는 바뀌지 않습니다.
작업공간 탭은 펼친 사이드바 오른쪽 앱 영역에 놓이며, 접으면 네이티브 탐색 버튼 다음에서
시작합니다. 좁은 네이티브 창에서는 두 영역에 모두 접근할 수 있도록 탭을 타이틀바 아래에
배치합니다. 사이드바 프로젝트 행에는 네이티브 토글을 중복 표시하지 않으며, 모바일 시트에는
닫기 토글을 유지합니다. 브라우저 데스크톱에서는 브라우저의 뒤로·앞으로 버튼을 사용합니다.

Worktrees 표시 옵션은 하단 메뉴 위의 빈 공간을 포함한 Worktrees 전체 영역에 마우스가
들어오면 나타납니다. 키보드 포커스, 열린 옵션 메뉴, 호버가 없는 입력 환경에서도 접근할 수 있습니다.

프로젝트·워크트리 탭 활성화와 사이드바 페이지 선택은 브라우저 세션 방문 기록을 사용합니다.
네이티브 뒤로·앞으로는 브라우저 탐색과 같은 History API로 작업공간 탭을 가로질러 이동합니다.
...
너비와 네이티브 toolbar 위치를 보존합니다. 앱이 viewport 높이를 소유하고 증거·목록은
각 panel에서 스크롤합니다. Flex/grid 경계에 `min-w-0`, 중첩 스크롤에는 `min-h-0`가 필요합니다.

작업공간 ID는 프로젝트·워크트리 ID와 독립적입니다. 각 탭은 페이지와 대상을 기록하고,
사이드바 이동은 활성 탭을 갱신하며 제목은 현재 위치를 반영합니다. 새 탭은 현재 위치를
새 ID로 복사합니다. 프로젝트·워크트리 변경은 활성 탭 ID를 유지하며 페이지 이름이
같아도 방문 기록에 목적지를 추가합니다. 새 탭과 닫기 동작은
