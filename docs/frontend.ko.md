...
소유하며 현재 페이지 제목이나 history 제어를 반복하지 않습니다. 작업공간 탭은 아이콘과 제목을 왼쪽 정렬하고 얇은 윤곽선과 밝은 선택 배경을 사용합니다.
옆의 더하기 버튼은 현재 페이지 위치를 상세 상태가 초기화된 새 탭으로 열며 가로 스크롤 중에도 접근할 수 있습니다.
탭 선택이나 닫기 후에는 활성 탭이 보이도록 이동합니다. 작업공간 탭은 고정 폭을
쓰고 긴 이름은 말줄임 처리하며 공간이 부족하면 가로로 스크롤합니다. 브라우저 데스크톱과
native macOS는 사이드바 전환을 제공하고, 좁은 모바일 화면에서도 사이드바 전환을
유지합니다. 모바일과 native 레이아웃에서
열린 사이드바의 전환 버튼은 프로젝트 선택 옆, 프로젝트 표시 옵션 바로 왼쪽에 있습니다.
닫힌 상태에서는 작업공간 헤더 아래에 다시 열기 버튼이 나타납니다. Worktrees 표시 옵션은
하단 메뉴 위의 남은 빈 공간을 포함한 Worktrees 전체 영역에 마우스가 들어오면 나타납니다.
키보드 포커스, 열린 옵션 메뉴, 호버가 없는 입력 환경에서도 버튼에 접근할 수 있습니다. 사이드바는 canvas에 남습니다. 데스크톱 gutter, 접힘 상태, 모바일 전체
native macOS는 창 버튼 옆 48px 타이틀바에 사이드바 토글과 뒤로·앞으로 버튼을 배치하고,
사이드바를 접어도 접근할 수 있습니다. 브라우저 데스크톱은 앱의 뒤로·앞으로 버튼을 생략하고
브라우저 버튼을 사용합니다.

프로젝트·워크트리 탭 활성화와 사이드바 페이지 선택은 브라우저 세션 방문 기록을 사용합니다.
macOS 뒤로·앞으로는 브라우저 탐색과 같은 History API로 작업공간 탭을 가로질러 이동합니다.
같은 화면을 다시 선택해도 중복 기록을 추가하지 않으며, 뒤로 이동한 뒤 새 화면을 선택하면
앞으로 기록을 지웁니다. 방문 기록은 목적지 페이지를 복원하고, 프로젝트가 연결되어 있으면
닫았던 목적지 탭도 다시 엽니다. 새로고침은 현재 목적지를 복원하지만 편집 초안이나 다른 탭은
복원하지 않습니다. 파일 선택, 리뷰 하위 탭, 스크롤 변경은 방문 기록을 추가하지 않습니다.
History API는 이동 가능 여부 전체를 제공하지 않으므로 네이티브 버튼은 활성 상태를 유지하며,
방문 기록의 끝에서는 이동해도 변화가 없습니다. 사이드바는 canvas에 남습니다. 데스크톱 gutter, 접힘 상태, 모바일 전체
너비와 macOS toolbar 위치를 보존합니다. 앱이 viewport 높이를 소유하고 증거·목록은
각 panel에서 스크롤합니다. Flex/grid 경계에 `min-w-0`, 중첩 스크롤에는 `min-h-0`가 필요합니다.

작업공간 ID는 프로젝트·워크트리 ID와 독립적입니다. 각 탭은 페이지와 대상을 기록하고,
사이드바 이동은 활성 탭을 갱신하며 제목은 현재 위치를 반영합니다. 새 탭과 닫기 동작은
[UX 탐색](ux-navigation.md)을 참고하세요. 열린 탭은 React Activity로 React 상태와 DOM을
유지합니다. 숨긴 탭은 effect 구독을 해제하고 다시 활성화하면 화면 상태를 유지한 채
조회합니다. Sidebar provider는 각 탭에 속합니다. 상태는 세션에만 유지되고 탭을 닫으면
해제되며, 새로고침 후 작성 중 입력을 복원하지 않습니다.
...
열린 작업공간 탭은 React Activity로 React 상태와 DOM을 유지합니다. 숨긴 탭은 effect
구독을 해제하고 다시 활성화할 때 화면 상태를 유지한 채 조회를 재개합니다. Sidebar
provider는 각 탭에 속합니다. 상태는 세션 중 탭을 닫을 때까지 유지하며 새로고침 후
작성 중인 입력을 복원하지는 않습니다.

프로젝트 선택 메뉴는 활성 작업공간에만 마운트하여 숨겨진 탭의 포털 메뉴가
남지 않도록 합니다.
