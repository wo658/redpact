...
임시 Compose 환경을 시작하기 전에 암시적인 이름의 빌드 이미지를 사용하는 선택된
서비스 목록을 이미지 정리용 manifest에 캡처합니다. 이미지 정리는 원본 Compose 파일,
비활성 서비스, 필수 환경변수 표현식을 다시 평가하지 않고 이 목록을 사용합니다.
애플리케이션 자격증명을 다시 제공할 필요가 없으며, 명시적으로 이름을 지정한 이미지는
이 manifest에서 제외합니다.
...
Compose가 로컬 이름으로 만든 빌드 이미지만 제거합니다. Redpact는 프로젝트 Compose 파일에
명시한 이미지나 사용자의 관계없는 Docker 이미지를 제거하지 않습니다.
